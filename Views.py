...
def insertadmin(request):
    ...
    s="insert into admin1 values('"+request.GET["Email"]+"','"+request.GET["Password"]+"','"+request.GET["Mobileno"]+"',0)"
    cr=conn.cursor()
    cr.execute(s)
    conn.commit()
    d={"message":"Admin added successfully"}
...
def insertcategory(request):
    ...
    conn=Connect("127.0.0.1","root","","goodreads")
    s="insert into category values('"+request.POST["catname"]+"','"+request.POST["description"]+"','"+uploadname+"')"
    cr=conn.cursor()
    cr.execute(s)
    conn.commit()
...
def save(request):
    ...
    if file!="":
        ...
        file = request.FILES["photo"]
        uploadname = ("categoryphotos/" + str(random.randint(1, 10000)) + file.name)
        s="update category set description='"+request.POST["description"]+"',catphoto='"+uploadname+"' where catname='"+request.POST["catname"]+"'"
        fs = FileSystemStorage()
        fs.save(uploadname,file)
        cr=conn.cursor()
        cr.execute(s)
        ...
        s1="update category set description='"+request.POST["description"]+"' where catname='"+request.POST["catname"]+"'"
        cr=conn.cursor()
        cr.execute(s1)
        conn.commit()
...
def insertbook(request):
    ...
    conn = Connect("127.0.0.1", "root", "", "goodreads")
    s = "insert into books values(NULL,'" +request.POST["title"]+"','"+request.POST["description"]+"',"+request.POST["price"]+",'"+request.POST["edition"]+"','"+request.POST["author"]+"','"+request.POST["genre"]+"','"+request.POST["catname"]+"','"+name+"')"
    fs = FileSystemStorage()
    fs.save(name, file)
    cr = conn.cursor()
    cr.execute(s)
